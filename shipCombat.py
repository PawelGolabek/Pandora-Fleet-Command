...
def drawShips(canvas,globalVar,uiMetrics):  # draw ship on the map with all of its accesories
    for ship in globalVar.ships:
        if(ship.visible or not globalVar.fogOfWar or ship.owner == "player1"):
            drawX = (ship.xPos - globalVar.left) * globalVar.zoom   # get coords relative to window
            drawY = (ship.yPos - globalVar.top) * globalVar.zoom

            fillColor = "red"
            if(ship.owner == "player1"):
                if(globalVar.shipChoice == ship.id):
            ...
            list = []
            ghostShip = dynamic_object()
            ghostShip.x = (ship.xPos - globalVar.left) * globalVar.zoom
            ghostShip.y = (ship.yPos - globalVar.top) * globalVar.zoom
            list.append(ghostShip)
            ghostShip = dynamic_object()
            ghostShip.x = (ship.xPos + uiMetrics.canvasWidth - globalVar.left) * globalVar.zoom
            ghostShip.y = (ship.yPos - globalVar.top) * globalVar.zoom
            list.append(ghostShip)
            ghostShip = dynamic_object()
            ghostShip.x = (ship.xPos - uiMetrics.canvasWidth - globalVar.left) * globalVar.zoom
            ...
            ghostShip = dynamic_object()
            ghostShip.x = (ship.xPos - globalVar.left) * globalVar.zoom  
            ghostShip.y = (ship.yPos + uiMetrics.canvasHeight - globalVar.top) * globalVar.zoom
            list.append(ghostShip)
            ghostShip = dynamic_object()
            ghostShip.x = (ship.xPos - globalVar.left) * globalVar.zoom 
            ghostShip.y = (ship.yPos - uiMetrics.canvasHeight - globalVar.top) * globalVar.zoom
